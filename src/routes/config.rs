...
/// Get current configuration (sensor 0)
...
pub async fn set_sample_rate(
    ...
    let Some(sensor) = state.sensors.get(0) else {
        return handle_no_device().await.into_response();
    };
...
pub async fn set_baud_rate(
    ...
    if ![115200u32, 3000000].contains(&payload.baud_rate) {
        return (
...
pub async fn set_stream_size(
    ...
    let guard = sensor.client.read().await;
    match &*guard {
        Some(client) => match client.set_stream_size(payload.stream_size).await {
            Ok(()) => {
...
async fn read_current_config(state: &AppState) -> anyhow::Result<ConfigResponse> {
    ...
    match &*guard {
        Some(client) => {
            ...
            let baud_rate = state.config.sensors.first().map(|s| s.baud_rate).unwrap_or(115200);
            Ok(ConfigResponse {
                sample_rate: 7812,
                baud_rate,
